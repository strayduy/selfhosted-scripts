...
echo -e "${YELLOW}2. Add your SSH public key to /home/$USERNAME/.ssh/authorized_keys BEFORE disconnecting${NC}"
echo -e "${YELLOW}3. Test SSH connection with new user and port BEFORE closing this session${NC}"
echo -e "${YELLOW}4. Password authentication is DISABLED${NC}"
echo -e "${YELLOW}5. Root login is DISABLED${NC}"
echo -e "${YELLOW}6. Only user '$USERNAME' can SSH to this server${NC}"
echo -e "${YELLOW}7. Automatic security updates are ENABLED${NC}"
echo -e "${YELLOW}8. sudo commands are fully logged to /var/log/sudo.log${NC}"
if [ "$INSTALL_TAILSCALE" = true ] && [ "$TS_SSH" = true ] && [ -n "$TS_AUTHKEY" ]; then
    echo -e "${YELLOW}9. Public SSH port is CLOSED — connect via: ssh $USERNAME@<tailscale-hostname>${NC}"
    echo -e "${YELLOW}   Tailscale must be authenticated on your client device${NC}"
fi
if [ "$INSTALL_TAILSCALE" = true ] && [ "$TS_SSH" = true ]; then
    echo -e "${YELLOW}10. Tailscale SSH bypasses sshd entirely — PermitRootLogin has no effect over the tailnet.${NC}"
